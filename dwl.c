...
typedef struct {
	...
	struct wl_listener set_hidden; // tambahan
...
static void focusstack_hd(const Arg *arg);
...
static void invisibleclient(const Arg *arg);
...
arrange(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link)
		if (c->mon == m)
			wlr_scene_node_set_enabled(&c->scene->node, VISIBLEON(c, m) && !c->ishidden);
...
focusstack(const Arg *arg)
{
	...
	if (arg->i > 0) {
		wl_list_for_each(c, &sel->link, link) {
			...
			if (VISIBLEON(c, selmon) && !c->ishidden)
				break; /* found it */
		}
	} else {
...
tile(Monitor *m)
{
	unsigned int i, n = 0, mw, my, ty, eg = enablegaps;
	int doublegaps = 2 * gaps;
	Client *c;
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			resize(c, (struct wlr_box){
					.x = m->w.x + gaps*eg, 
					.y = m->w.y + my + gaps*eg, 
					.width = mw - doublegaps*eg,
					.height = (m->w.height - my) / (MIN(n, m->nmaster) - i) - doublegaps*eg}, 0);
			my += c->geom.height + gaps*eg;
		} else {
			resize(c, (struct wlr_box){
					.x = m->w.x + mw, 
					.y = m->w.y + ty + gaps*eg,
					.width = m->w.width - mw - gaps*eg, 
					.height = (m->w.height - ty) / (n - i) - doublegaps*eg}, 0);
			ty += c->geom.height + gaps*eg;
		}
		i++;
	}
}
